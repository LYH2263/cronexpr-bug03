...
// Stage8 holds planner-side helpers for cron field introspection.
type Stage8 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage8) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage8"
	}
