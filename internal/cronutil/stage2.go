...
// Stage2 holds planner-side helpers for cron field introspection.
type Stage2 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage2) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage2"
	}
