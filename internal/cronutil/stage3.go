...
// Stage3 holds planner-side helpers for cron field introspection.
type Stage3 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage3) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage3"
	}
