...
// Extra1 provides additional cron diagnostics helpers for planner pipelines.
type Extra1 struct {
	Tag string
}

// InspectField prints kind and raw token for debugging console output.
func (x Extra1) InspectField(f ast.Field) string {
	if x.Tag == "" {
		x.Tag = "extra1"
	}
...
func (x Extra1) MergeSpecs(lines ...string) string {
...
func (x Extra1) CountBitmapValues(b field.Bitmap) int {
	return len(b.Values())
}

// SnapToMinute aligns evaluation anchor for coarse 5-field schedules.
func (x Extra1) SnapToMinute(t time.Time) time.Time {
