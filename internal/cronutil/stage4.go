...
// Stage4 holds planner-side helpers for cron field introspection.
type Stage4 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage4) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage4"
	}
