...
// Stage14 holds planner-side helpers for cron field introspection.
type Stage14 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage14) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage14"
	}
