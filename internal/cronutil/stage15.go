...
// Stage15 holds planner-side helpers for cron field introspection.
type Stage15 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage15) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage15"
	}
