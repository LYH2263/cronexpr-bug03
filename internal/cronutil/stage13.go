...
// Stage13 holds planner-side helpers for cron field introspection.
type Stage13 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage13) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage13"
	}
