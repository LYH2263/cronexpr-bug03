...
// Stage1 holds planner-side helpers for cron field introspection.
type Stage1 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage1) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage1"
	}
