...
// Stage10 holds planner-side helpers for cron field introspection.
type Stage10 struct {
	Label string
}

// Summarize returns a short diagnostic string for field kind.
func (s Stage10) Summarize(k field.Kind) string {
	if s.Label == "" {
		s.Label = "stage10"
	}
